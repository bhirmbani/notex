...
// its own ~120s budget. Unlike expandForDraft.ts, there is no proxy fallback — ADR-0007 accepts
...
//
// The provider call, budget, and citation-validation contract live in citedSynthesis.ts, shared
// with synthesizeNodeExplanation.ts (TBR-114) — this file only builds the question+evidence
// prompt and supplies its own source list and citation failure message.

import { SYNTHESIS_MAX_TOKENS, SYNTHESIS_TIMEOUT_MS, runCitedSynthesis, sourceKey } from "./citedSynthesis"
import type { CitedSynthesisOutcome } from "./citedSynthesis"
import type { ProviderConfig } from "@/features/provider-keys/types"

export { SYNTHESIS_MAX_TOKENS, SYNTHESIS_TIMEOUT_MS }
...
export type DraftSynthesisOutcome = CitedSynthesisOutcome
...
export async function synthesizeForDraft(
  ...
  return runCitedSynthesis(
    ...
    prompt,
    context.sources,
    "synthesized prose cites a source not in context.sources"
  )
}
