...
function appWithAuth() {
  ...
  app.route('/', mindmapApi)
  return app
}

describe('POST /organizations/:organizationId/projects/:projectId/links', () => {
...
describe('DELETE /organizations/:organizationId/links/:id', () => {
  it('returns 404 when the link does not exist', async () => {
    ...
    const app = appWithAuth()
    const res = await app.request('/organizations/org-1/links/missing', { method: 'DELETE' }, {})

    expect(res.status).toBe(404)
  })

  it('deletes the link for a member of the project org', async () => {
    const link = { id: 'link-1', projectId: 'project-1' }
    const project = { id: 'project-1', organizationId: 'org-1' }
    const membership = { id: 'membership-1', organizationId: 'org-1', userId: 'user-1', role: 'admin' }
    let call = 0
    const results = [[link], [project], [membership]]
    ...
    const app = appWithAuth()
    const res = await app.request('/organizations/org-1/links/link-1', { method: 'DELETE' }, {})
