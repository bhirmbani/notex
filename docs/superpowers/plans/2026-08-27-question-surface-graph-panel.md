...
Note: `vi.mock` factories run before other imports are evaluated, so the `vi.mock("@tanstack/react-router", ...)` call must appear before the `import { QuestionGraphAction } from "./QuestionGraphAction"` line, as shown above (vitest hoists `vi.mock` calls to the top of the file automatically, but keep the physical ordering as written for readability).
