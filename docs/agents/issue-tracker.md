...
- **Open a stacked PR per sub-issue as soon as its branch is committed** — don't batch PR creation until the end. The PR's base branch is whatever branch it was actually created *from* in git (its blocker's branch), not always `main`:
  ...
  - Merge sequentially, oldest first. Once a base PR merges to `main` (with its branch deleted), GitHub auto-retargets any PR whose base was that branch to `main` — there is no need to manually rebase or retarget every downstream PR after each merge.
